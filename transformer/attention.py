...
class MultiHeadAttention(nn.Module):
    """
    多头注意力机制模块
    将输入分成多个头进行并行注意力计算，增强模型对不同子空间的表达能力。
    """
    ...
    def forward(self, Q: torch.Tensor, K: torch.Tensor, V: torch.Tensor, mask=None) -> torch.Tensor:
        ...
        Args:
            Q: Query 向量，形状 (batch_size, seq_len, d_model)
            K: Key 向量，形状 (batch_size, seq_len, d_model)
            V: Value 向量，形状 (batch_size, seq_len, d_model)
            mask: 可选的掩码张量
